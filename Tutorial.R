...
#' 303 - 351          Section 2.1  Optimizing 2-sample DGE analysis
#' 352 - XXX          Section 2.2  Optimizing multiple sample DGE analysis *(UNDER CONSTRUCTION)*
...
#'*Sum_w^2*      sum of sample weight squares, *used for advanced quality control, see ADVANCED TUTORIAL*.
#
...
#
# In my experience, the most common problems encountered in scRNASeq are
# 1. Low number of cells of interest (particularly in studies of progenitor cells or rare cell types)
# 2. Low expression of genes of interest
# From the perspective of statistical analysis, these are just two sides of the same problem: not enough cells expressing the genes of interest for accurate statistics.
# Of course, the best solution is always to perform more experiments with higher sequencing depth and hope for the best. However, this is expensive and not always feasible.
# An alternative solution is to analyze genes expressed in a borderline number of cells at the cost lower analysis accuracy and then validate the results by much cheaper
# independent experiments. The purpose of this section is to describe how this can be done by optimizing analysis parameters while monitoring quality control output parameters.
#
##################################################################################
#'*Section 2.1 Optimizing 2-sample DGE analysis*
#
# The key DGE.2samples() parameters that can be optimized are: min.count= and min.cells=.
# The min.pct= parameter is useful only for comparing with other types of analysis that use this parameter (e.g., methods built into Seurat).
# The sum.w2= parameter is much trickier and may be useful only for more advanced users.
#
# For instance, if we use min.cells=5 instead of default 10 and min.count=10 instead of default 30
DGE.results.mc5=DGE.2samples(Neuron.QC, ident.1 = "glutcells", ident.2 = "GABAcells", min.cells=5, min.count = 10)
# the analysis of the same dataset reports almost 2,000 more genes than in DGE.Results calculated by DGE.2samples() at default settings. 
diff.genes=setdiff(rownames(DGE.results.mc5),rownames(DGE.results))
# identifies these "extra" 1,873 low expression genes
DGE.results.dg=DGE.results.mc5[diff.genes,]
# is the dataframe containing just these low expression genes. Note that 183 of these genes appear to be upregulated or downregulated more than 16-fold:
DGE.results.LE=DGE.results.dg[abs(DGE.results.dg$log2FC)>4,]
# but is this real DGE? Some of these 183 findings could result from weighted averaging inaccuracy at low min.cell and min.count settings. The reason behind the default
# min.cells=10 and min.count=30 settings is that accurate weighted averaging requires large number of cells and large number of counts. It is nearly impossible to estimate 
# how relative errors depend on the values of these numbers. All we can really say is that smaller values of these numbers may lead to larger errors. Therefore, we must proceed 
# with caution and validate answers by alternative analysis methods and additional experiments.
#
# With this in mind, we can test which of the 183 genes in DGE.results.LE are most likely to be relevant by using common non-weighted averaging and common, non-weighted t-test.
# The latter are more conservative than the weighted averaging and t-test and require fewer cells and counts. To do that, we reanalyze these genes with the icc=1 setting 
DGE.results.LEgenes=DGE.2samples(Neuron.QC, features=rownames(DGE.results.LE), ident.1 = "glutcells" ,ident.2 = "GABAcells", min.cells=5, min.count = 10, icc=1)
...
# Turns out that 174 of the 183 genes meet these requirements. Moreover, very conservative p<0.0001 in the conservative t-test
DGE.results.LEgenes.b=DGE.results.LEgenes.a[DGE.results.LEgenes.a$p.value<0.0001,]
# suggests that at least 20 of these genes are indeed highly likely to have significant DGE.
#    If we were confident enough in the lack of cell sampling bias to analyze just 0.3% of our cells, we could push the analysis even further, to the 3-cell limit of the 
# non-weighted t-test and no min.count requirement:
DGE.results.mc3=IterWghtTtest(Neuron.QC, ident.1 = "glutcells", ident.2 = "GABAcells", min.cells=3, min.count = 0)
# Here, IterWghtTest() performs the same weighted averaging and t-test as DGE.2samples() but does not perform the chi squared test, which is meaningless at low counts. 
# The t-test followup
diff.genes.mc3=setdiff(rownames(DGE.results.mc3),rownames(DGE.results))
DGE.results.dg3=DGE.results.mc3[diff.genes.mc3,]
DGE.results.LE3=DGE.results.dg3[abs(DGE.results.dg3$log2FC)>4,]
DGE.results.LEgenes3=IterWghtTtest(Neuron.QC, features=rownames(DGE.results.LE3), ident.1 = "glutcells" ,ident.2 = "GABAcells", min.cells=3, min.count = 0, icc=1)
DGE.results.LEgenes.c=DGE.results.LEgenes3[(abs(DGE.results.LE3$log2FC*DGE.results.LEgenes3$log2FC)>0&
                                             (abs(DGE.results.LE3$log2FC-DGE.results.LEgenes3$log2FC)<0.37|
                                                (abs(DGE.results.LE3$log2FC)==Inf&abs(DGE.results.LEgenes3$log2FC)==Inf))),]
DGE.results.LEgenes.d=DGE.results.LEgenes.c[DGE.results.LEgenes.c$p.value<0.0001,]
# still finds the same 20 genes with p<0.0001 but 196 genes with p<0.01. Can we afford to miss these genes with abs(log2FC)>4? The answer depends on the questions being
# asked by the study and efforts needed to validate these genes as relevant and truly differentially expressed. It is worth remembering that scRNASeq usually identifies
# between 1 and 5 % of all transcripts in the cell. Genes detected in just 0.5% of the cells may therefore be actually expressed in 10-50% of the cells. Are these genes
# relevant? The answer may be in the eye of the beholder. Check out the values of N.+_cells and Counts in the output dataframe and decide whether the validation would
# worthwhile for you. This why these quality control output parameters are calculated.
#
##################################################################################
