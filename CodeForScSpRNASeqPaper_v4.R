...
results.SS<-data.frame(FM001=numeric(10), FMT001=numeric(10), T001=numeric(10), CHIIWT001=numeric(10),
                       SCT001=numeric(10), FMSCT001=numeric(10), SCTc001=numeric(10), FMSCTc001=numeric(10))
for (sim in c(1:10)){
  for (j in c(1:Ng)) {
    set.seed((sim-1)*Ng+j)
  ...
  GLUTc.QC<-JoinLayers(merge(GLUTo.QC, y=c(GLUTs.QC), add.cell.ids = c("C1","C2"), merge.data = "FALSE")) #concatenate original and subsampled data
  GLUTc.QC[["percent.mt"]] <- PercentageFeatureSet(GLUTc.QC, pattern = "^mt-")
  GLUTs.QC[["percent.mt"]] <- PercentageFeatureSet(GLUTs.QC, pattern = "^mt-")
  GLUTo.QC[["percent.mt"]] <- PercentageFeatureSet(GLUTo.QC, pattern = "^mt-")
  GLUTc.SCT<-SCTransform(GLUTc.QC,vars.to.regress = "percent.mt", verbose = FALSE)
  GLUTc.SCT[["RNA"]]<-NULL
  GLUTc.SCT<-RenameAssays(object = GLUTc.SCT, SCT="RNA")
  GLUTcs.SCT<-subset(GLUTc.SCT,idents = "Subset")
  GLUTco.SCT<-subset(GLUTc.SCT,idents = "Orig")
  GLUTcs.SCT.mat<-as.matrix(GLUTcs.SCT[["RNA"]]$counts)
  GLUTco.SCT.mat<-as.matrix(GLUTco.SCT[["RNA"]]$counts)
  GLUTcs.SCT<-CreateSeuratObject(counts = GLUTcs.SCT.mat, min.cells = 0, min.features = 0)
  GLUTco.SCT<-CreateSeuratObject(counts = GLUTco.SCT.mat, min.cells = 0, min.features = 0)
  Idents(GLUTcs.SCT)<-"Subset"
  Idents(GLUTco.SCT)<-"Orig"
  GLUTc.QC<-JoinLayers(merge(GLUTco.SCT, y=c(GLUTcs.SCT), add.cell.ids = c("C1","C2"), merge.data = "FALSE")) #merging Seurat objects 
  GLUTs.SCT<-SCTransform(GLUTs.QC,vars.to.regress = "percent.mt", verbose = FALSE, scale_factor=23000)
  GLUTo.SCT<-SCTransform(GLUTo.QC,vars.to.regress = "percent.mt", verbose = FALSE, scale_factor = 23000)
  ...
  GLUTc.QCN<-NormalizeData(GLUTc.QC)
  ...
  genes.c<-rownames(GLUTc.QCN[["RNA"]]$counts)
  genes.c=intersect(genes.c,genes.s)
  ...
  GLUTc.FM.SCT <- as.data.frame(FindMarkers(GLUTc.QCN, features=genes.c ,ident.1 = "Orig",ident.2 = "Subset",logfc.threshold = 0,min.pct=0))
  ...
  GLUTc.DGE.SCT <- DGE.2samples(GLUTc.QCN, features = genes.c, ident.1 = "Orig",ident.2 = "Subset", min.count=0, max.pval = 1) #SCT+weighted-t/chi^2-test
  #Simulation results
  ...
  GLUTc.FM.SCT.001<-GLUTc.FM.SCT[GLUTc.FM.SCT$p_val<0.001,]
  GLUTc.DGE.SCT.001<-GLUTc.DGE.SCT[((GLUTc.DGE.SCT$p.value<0.001)&(GLUTc.DGE.SCT$Chi2.p.value<0.05)),]
  ...
  results.SS$SCTc001[sim]=nrow(GLUTc.DGE.SCT.001)
  results.SS$FMSCTc001[sim]=nrow(GLUTc.FM.SCT.001)
}
Idents(GLUTco.SCT)="Orig.SCT"
Idents(GLUTcs.SCT)="Subset.SCT"
GLUT.conc.SCT=JoinLayers(merge(GLUTo.QC, y=c(GLUTs.QC,GLUTcs.SCT,GLUTco.SCT), add.cell.ids = c("C1","C2","C3","C4"), merge.data = "FALSE"))
GLUTco.WT.SCT=IterWghtTtest(GLUT.conc.SCT, features = genes.c, ident.1 = "Orig",ident.2 = "Orig.SCT", min.count=0, max.pval = 1, icc = "i")
GLUTcs.WT.SCT=IterWghtTtest(GLUT.conc.SCT, features = genes.c, ident.1 = "Subset",ident.2 = "Subset.SCT", min.count=0, max.pval = 1, icc = "i")
#Saving results as csv files
write.csv(GLUTco.WT.SCT,"GLUTco.WT.SCT.40%.csv")
write.csv(GLUTcs.WT.SCT,"GLUTcs.WT.SCT.40%.csv")
...
write.csv(GLUTc.DGE.SCT,"GLUTc.SS.2T.SCT.40%.csv")
write.csv(GLUTc.FM.SCT,"GLUTc.SS.FM.SCT.40%.csv")
write.csv(results.SS,"results.SSc2.40%.csv")
result
#
######################################################
