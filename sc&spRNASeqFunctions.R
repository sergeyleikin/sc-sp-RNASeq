...
###################################################################################################
###################################################################################################
# A little bit of testing and fun, 2-sample analysis
###################################################################################################
setwd("D:/RWorkDir/scRNASeq/")
Neuron.QC<-readRDS("Neuron.QC.rds") #reads Neuron.QC dataset for GABAergic and glutamatergic neurons assembled as 
# described in the paper. Quality control (QC) was based on 0.75 < percent.mt < 7.5 and nCount_RNA > 1000
# The QC thresholds were selected based on the appearance of the corresponding violin plots
Neuron.QCN<-NormalizeData(Neuron.QC, normalization.method = "RC")
ttest<-IterWghtTtest(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",min.pct = 0.03, max.pval = 0.01,icc=1)
ttest.genes<-rownames(ttest)
chi2test<-Chi2Test(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",min.pct = 0.03, max.pval = 0.01)
IWT.icc.i<-IterWghtTtest(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",min.pct = 0.03, max.pval = 0.01,icc="i")
IWT.icc.A<-IterWghtTtest(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",min.pct = 0.03, max.pval = 0.01,icc="A")
i.features<-setdiff(rownames(IWT.icc.i),rownames(IWT.icc.A))
A.features<-setdiff(rownames(IWT.icc.A),rownames(IWT.icc.i))
IWT.icc.A.i<-IterWghtTtest(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",features=i.features,min.pct = 0.03, max.pval = 1,icc="A")
test.A.i<-cbind(IWT.icc.A.i,IWT.icc.i[i.features,])
IWT.icc.i.A<-IterWghtTtest(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",features=A.features,min.pct = 0.03, max.pval = 1,icc="i")
test.i.A<-cbind(IWT.icc.i.A,IWT.icc.A[A.features,])
chi2.icc.iwt<-IWT.icc0[icc0.features,]
chi2.icc<-cbind(chi2.icc.iwt,chi2.icc.chi)
DGE2S<-DGE.2samples(Neuron.QC,ident.1 = "GLUT",ident.2 = "GABA",max.chi = 1,min.pct = 0.03, max.pval = 0.01)
FMttest<-as.data.frame(FindMarkers(Neuron.QCN,ident.1 = "GLUT",ident.2 = "GABA", test.use = "t",features=ttest.genes,logfc.threshold = 0,min.pct=0))
TT.FM<-cbind(ttest,FMttest[ttest.genes,]) #Check out log2FC for Col19a1, FindMarkers is very inaccurate because of pseudocount artifacts
length(setdiff(ttest.genes,rownames(DGE2S))) #t-test finds 520 genes with p < 0.01 DGE.2samples does not!
DGE2S.only<-setdiff(rownames(DGE2S),ttest.genes)
length(DGE2S.only) #DGE.2samples finds 784 genes with p < 0.01 t-test does not!
comm.genes<-intersect(ttest.genes,rownames(DGE2S))
TT.DGE2S<-cbind(ttest[comm.genes,],DGE2S[comm.genes,])
TT.DGE2SF<-TT.DGE2S[abs(TT.DGE2S[,1]-TT.DGE2S[,5])>1,]
fun<-TT.DGE2SF[!is.na(TT.DGE2SF[,1]),] #Among the common genes, 
#t-test gets fold-change wrong by more than a factor of 2 in 27 genes among common p < 0.01 between 
#the t-test and chi-squared/weighted-t combination because the t-test does not have weights.
#Some of them are not even low expression genes. This is a big deal and an additional reason why I cannot
#use t-test under any circumstances if weights can be determined.
TT.DGE2ST<-TT.DGE2S[(TT.DGE2S[,3]>50&TT.DGE2S[,4]>50),] #As expected, the results are very close for very high expression genes,
#which is comforting.
fun2<-DGE2S[DGE2S.only,]
fun2<-fun2[fun2[,2]<1e-4,] #55 genes detected by weighted t-test with p<0.0001 but not t-test with p<0.01
#Almost all of them are moderately or highly expressed genes. This is not a minor loss of power at all.
#####################################################################################################
#####################################################################################################
#
################################################################################################
################################################################################################
# Usage example for multiple sample analysis and comparison with 2 sample analysis of the same data
################################################################################################
setwd("D:/RWorkDir/scRNASeq")
NeuronMS.QC<-readRDS("NeuronMS.QC.rds")                             #read preassembled data file
test<-DGE.MultiSample(NeuronMS.QC,samples.1=c("GLUT.1","GLUT.2","GLUT.3"),samples.2=c("GABA.1","GABA.2","GABA.3"),fc.thr = 2,max.pval = 0.001,icc = "i") #results
test.m<-CntAv(NeuronMS.QC)
SM<-SampleMatrix(NeuronMS.m,samples.1=c("GLUT.1","GLUT.2","GLUT.3"),samples.2=c("GABA.1","GABA.2","GABA.3"))
test<-WT.MultiSample(SM,fc.thr = 2,max.pval = 0.001)
Neuron3S.QC<-NeuronMS.QC                                            #prepare for 2 sample analysis by changing identities
Neuron3S.QC<-RenameIdents(Neuron3S.QC, 'GLUT.1'="GLUT")
Neuron3S.QC<-RenameIdents(Neuron3S.QC, "GABA.1"="GABA")
Neuron3S.QC<-RenameIdents(Neuron3S.QC, 'GLUT.2'="GLUT")
Neuron3S.QC<-RenameIdents(Neuron3S.QC, "GABA.2"="GABA")
Neuron3S.QC<-RenameIdents(Neuron3S.QC, 'GLUT.3'="GLUT")
Neuron3S.QC<-RenameIdents(Neuron3S.QC, "GABA.3"="GABA")
test2S<-DGE.2samples(Neuron3S.QC,ident.1 = "GLUT",ident.2 = "GABA",fc.thr = 2,max.pval = 0.001) #2 sample analysis results
nrow(test)   #361 genes
nrow(test2S) #1788 genes 
com.genes<-intersect(rownames(test),rownames(test2S)) #360 common genes (of possible 361)
TwoS.MS <- setdiff(rownames(test2S),rownames(test)) #1428 genes, of which 421 genes have 0 counts in at least 1 of the 6 samples
# and cannot be analyzed reliably, 1004 have p value above 0.001, 2 genes have mean value of min.pct < 0.03, and 1 has log2FC=-0.99
testMS.2S<-DGE.MultiSample(NeuronMS.QC,features=TwoS.MS,samples.1=c("GLUT.1","GLUT.2","GLUT.3"),samples.2=c("GABA.1","GABA.2","GABA.3"),min.pct=0,fc.thr = 1,max.pval = 1)
testMS.2S.p<-testMS.2S[testMS.2S$p.value<0.001,]
MS.TwoS <- setdiff(rownames(test),rownames(test2S)) #1 gene log2FC=-0.9976
test2S.MS<-DGE.2samples(Neuron3S.QC,features=MS.TwoS,ident.1 = "GLUT",ident.2 = "GABA",fc.thr = 1,max.chi = 0.999,max.pval = 1)
TwoS.MS.common<-cbind(test[com.genes,],test2S[com.genes,])
#As expected, the results for log2FC are reasonably close, but p-values are much larger for proper multiple sample comparison  
#################################################################################################
#################################################################################################
#Comparison with pseudo-bulk/DESeq2 analysis
NeuronMS.AC<-matrix(0,nrow = nrow(NeuronMS.QC),ncol =length(levels(NeuronMS.QC@active.ident)))
rownames(NeuronMS.AC)<-rownames(NeuronMS.QC[["RNA"]]$counts)
samples<-levels(NeuronMS.QC@active.ident)
colnames(NeuronMS.AC)<-samples
for (i in c(1:ncol(NeuronMS.AC))) {
  NeuronMS.Ci<-as.matrix(subset(NeuronMS.QC, idents = samples[i])[["RNA"]]$counts)
  NeuronMS.AC[,i]<-rowSums(NeuronMS.Ci)
}
GLUT.AC<-CreateSeuratObject(counts = NeuronMS.AC[,c(1:3)], min.cells = 0, min.features = 0)
Idents(GLUT.AC)<-"GLUT"
GABA.AC<-CreateSeuratObject(counts = NeuronMS.AC[,c(4:6)], min.cells = 0, min.features = 0)
Idents(GABA.AC)<-"GABA"
Neuron.AC.MS<-JoinLayers(merge(GLUT.AC, y=GABA.AC, merge.data = "FALSE"))
Neuron.AC.MSN<-NormalizeData(Neuron.AC.MS, normalization.method = "RC")
test.DESeq2<-FindMarkers(Neuron.AC.MSN, ident.1 = "GLUT",ident.2 = "GABA",test.use="DESeq2", logfc.threshold = 0,min.pct=0.03)
test.DESeq2<-test.DESeq2[!is.na(test.DESeq2[,1]),]
test.DESeq2.001<-test.DESeq2[test.DESeq2[,1]<0.001,]
test.DESeq2.001<-test.DESeq2.001[(test.DESeq2.001[,2]<(-1))|(test.DESeq2.001[,2]>(1)),] #215 genes vs. 361 found by DGE.MultiSample!!!
MS.DES<-intersect(rownames(test),rownames(test.DESeq2.001)) #Only 90 common genes, 
MS.DES.TABLE<-cbind(test[MS.DES,],test.DESeq2[MS.DES,]) #huge differences in p-values and large differences in log2FC for most of them
DES.MS.dif<-setdiff(rownames(test.DESeq2.001),rownames(test))
DES.MS.dif.TABLE<-DGE.MultiSample(NeuronMS.QC,features=DES.MS.dif,samples.1=c("GLUT.1","GLUT.2","GLUT.3"),samples.2=c("GABA.1","GABA.2","GABA.3"),min.pct=0,fc.thr = 1,max.pval = 1)
DES.MS.dif.TABLE.01<-DES.MS.dif.TABLE[DES.MS.dif.TABLE[,2]>0.01,] #47 genes found by DESeq with p<0.001 that have p>0.01 in DGE.MultiSample 
MS.DES.dif<-setdiff(rownames(test),rownames(test.DESeq2.001))
MS.DES.dif.TABLE<-cbind(test[MS.DES.dif,],test.DESeq2[MS.DES.dif,]) #271 genes mostly due to large difference in log2FC, only 3 of them have p>0.01
#Conclusions: pseudo-bulk analysis dramatically distorts log2FC values and produces very large number of false positive and false negative results
###################################################################################################