...
# 5. Detected_N.cells.1 - Number of detected cells expressing the gene, ident 1 
# 6. Detected_N.cells.2 - Number of detected cells expressing the gene, ident 2 
# 7. Sum_w^2.1 - Sum of the squares of stat. weights, ident 1 
# 8. Sum_w^2.2 - Sum of the squares of stat. weights, ident 2 
# 9. Chi2.p.value - unadjusted p.value for aggregate counts in chi-squared test
#
#'* Quality control considerations:* 
# A. Small Detected_N.cells in both identities precludes reliable statistical inference (not enough data points);
#    min.cells parameter eliminates genes with Detected_N.cells<min.cells in both identities, default min.cells=10.
#    Detected_N.cells can be small or even zero in just one of the two identities without affecting the output accuracy.
...
# C. Borderline values of Detected_N.cells and/or Sum_w^2 in both samples are warning signs of low analysis accuracy.
#    
#############
...
#############
#'* DGE.Multisample(object,samples.1=NULL,samples.2=NULL,features=NULL,t.test=FALSE, min.pct=0.03,fc.thr=1,max.pval=1,icc="i",df.correction=FALSE,sum.w2=0.4,min.cells=10)*
...
# 6. Counts/cell.1 - average counts per cell, samples.1
# 7. Sum_w^2.1 - Sum of the squares of stat. weights of the samples, samples.1 (Quality control)
# 8-12. - same as 3-7 but for samples.2
#
#'* Quality control considerations: *
# A. Please see description of DGE.2Samples() for discussion of min.cells and sum.w2 parameters.
#    The same default settings are used in the multi-sample analysis.
# B. In the context of multi-sample analysis, a gene is excluded from the analysis when the number of samples with enough (>=min.cells) expressing the gene drops below 3 in both groups of samples. 
...
IterWghtTtest <- function(object,features=NULL,ident.1=NULL,ident.2=NULL,fc.thr=1,min.pct=0,max.pval=1,min.count=30,
  ...
  if (is.null(ident.1)|is.null(ident.2)) {stop("Two identities in a Seurat object must be defined using ident.1= and ident.2=")}
  output<-data.frame(log2FC=numeric(),p.value=numeric(), col3=numeric(),col4=numeric(),                  # output dataframe
                     col5=numeric(),col6=numeric(),col7=numeric(),col8=numeric())
  colnames(output)<-c("log2FC","p.value", "Counts/Cell.1","Counts/Cell.2",                               # output dataframe
                      "Detected_N.cells.1","Detected_N.cells.2","Sum_w^2.1","Sum_w^2.2")
  ...
  for (rownum in c(1:length(GeneList))) {                                                                # Main test loop
    ...
    if ((AC.1 >= min.count | AC.2 >= min.count)&                                                         # checking for minimum aggregated counts
      ...
      icc.1=ifelse(icc==1,1,ifelse(Ncd.1>=5,icc,0))                                                      # setting icc=0 for genes expressed in fewer than 5 cells, unless the initial setting is icc=1
      icc.2=ifelse(icc==1,1,ifelse(Ncd.2>=5,icc,0))
      wi.1<-ICCWeight(h=Ci.1[GeneList[rownum],],n=Ni.1,icc=icc.1)                                        # ident.1 weights
      wi.2<-ICCWeight(h=Ci.2[GeneList[rownum],],n=Ni.2,icc=icc.2)                                        # ident.2 weights
      ...
      if (!is.na(fc)){                                                                                   # removing 0/0 division
        if ((fc >= fc.thr | fc <= 1/fc.thr)) {                                                           # checking FC threshold                                                      
          ...
          if (wTtest <= max.pval) {                                                                      # checking p-value threshold    
            ...
            output[[rowi,7]] <- as.numeric(format(wi.1sq, digits=4, scientific=FALSE))
            output[[rowi,8]] <- as.numeric(format(wi.2sq, digits=4, scientific=FALSE))
...
CntAv<-function(object,features=NULL,icc="i"){
  ...
  for (j in c(1:Ns)){                                                                    # main loop for calculating weighted average and variance sample by sample
    ...
    for (i in c(1:Ng)){                                                                  # gene by gene calculation subloop 
      ...
      icci=ifelse(icc==1,1,ifelse(Nd>=5,icc,0))                                          # setting icc=0 for genes expressed in fewer than 5 cells, unless the initial setting is icc=1
      w<-ICCWeight(h=h,n=Ni,icc=icci)                                                    # statistical weights
    ...
    AVdat<-cbind(AV,VAR,PCT,CNT)                                                         # output matrix assembly
...
SampleMatrix<-function(object,samples.1=NULL,samples.2=NULL){
  ...
  samples.matrix<-list(AV.1,VAR.1,AV.2,VAR.2,PCT.1,PCT.2,CNT.1,CNT.2,Sstats=Sstats)      # assembling a single list of sample matrices for
...
WT.MultiSample <- function(sample.matrix,features=NULL,t.test=FALSE ,min.pct=0.03,fc.thr=1,max.pval=1,df.correction=FALSE,sum.w2=0.4,min.cells=10) {
  ...
  Ncells1=Sstats["N.cells",c(1:N1)]                                                  # number of cells per sample, group 1
  Ncells2=Sstats["N.cells",c(N1:(N1+N2))]                                            # number of cells per sample, group 2
  output<-data.frame(p.value=numeric(),log2FC=numeric(),Av1=numeric(),Sd1=numeric(),C1=numeric(),CN1=numeric(),W1=numeric(),
                     Av2=numeric(),Sd2=numeric(),C2=numeric(),CN2=numeric(),W2=numeric())
  colnames(output)<-c("log2FC","p.value","Wtd.%UMI.1","Sd.%UMI.1","Av.cell.fr.1","Counts/cell.1","Sum_w^2.1",
                      "Wtd.%UMI.2","Sd.%UMI.2","Av.cell.fr.2","Counts/cell.2","Sum_w^2.2")
  ...
  for (rownum in c(1:length(GeneList))) {                                            # gene by gene DGE calculation loop
    ...
    Nc1=round(PCTi1*Ncells1)                                                         # number of cells per sample that express the gene, group 1
    Nc2=round(PCTi2*Ncells2)                                                         # number of cells per sample that express the gene, group 2
    Ns1=sum(Nc1>=min.cells)
    Ns2=sum(Nc2>=min.cells)
# selecting genes meeting minimum sample and expression requirements    
    if (N1 >= 3 & N2 >= 3 & (mean(PCTi1)>=min.pct|mean(PCTi2)>=min.pct) & (Ns1>=3|Ns2>=3)){
...
#                     (c) Number of samples with enough cells (>=min.cells) expressing the gene >= 3 in one of 2 groups ((sum(Nc1>=min.cells)>=3)|(sum(Nc2>=min.cells)>=3))
        ...
        if (fc >= fc.thr | fc <= 1/fc.thr) {                                         # checking FC threshold
          ...
          if(!is.na(wTtest)) {                                                       # output filtering for wTtest=NaN
            if (wTtest <= max.pval) {                                                # output filtering for p-value
              ...
              output[[rowi,6]] <- as.numeric(format(mean(CNTi1), digits=4, scientific=FALSE))
              output[[rowi,7]] <- as.numeric(format(mean(wi.1sq), digits=4, scientific=FALSE))
              output[[rowi,8]] <- as.numeric(format(Xi2av, digits=4, scientific=FALSE))
              output[[rowi,9]] <- as.numeric(format(Xi2sd, digits=4, scientific=FALSE))
              output[[rowi,10]] <- as.numeric(format(mean(PCTi2), digits=4, scientific=FALSE))
              output[[rowi,11]] <- as.numeric(format(mean(CNTi2), digits=4, scientific=FALSE))
              output[[rowi,12]] <- as.numeric(format(mean(wi.2sq), digits=4, scientific=FALSE))
              row.names(output)[rowi]<-GeneList[rownum]
            } 
          }
          else {
