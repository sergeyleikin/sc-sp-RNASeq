...
Chi2Test <- function(object,features=NULL,ident.1=NULL,ident.2=NULL,fc.thr=1,min.pct=0,max.pval=1,min.count=30) {
  if (is.null(features)){
    ...
    GL <- intersect(as.vector(features),rownames(object))
  }
  OrphanGenes=setdiff(as.vector(features),rownames(object))
  ...
  if (length(OrphanGenes)>0) {
    warning(paste0(toString(OrphanGenes), " gene(s) were not found in the object."))  
  }
  return(output)
}
...
IterWghtTtest <- function(object,features=NULL,ident.1=NULL,ident.2=NULL,fc.thr=1,min.pct=0,max.pval=1,min.count=30,
  ...
  if (is.null(features)){
    ...
    GeneList <- intersect(as.vector(features),rownames(object))
  }
  OrphanGenes=setdiff(as.vector(features),rownames(object))
  if (is.null(ident.1)|is.null(ident.2)) {stop("Two identities in a Seurat object must be defined using ident.1= and ident.2=")}
  output<-data.frame(log2FC=numeric(),p.value=numeric(), col3=numeric(),col4=numeric(),                  # output dataframe
                     col5=numeric(),col6=numeric(),col7=numeric(),col8=numeric(),col9=numeric(),col10=numeric())
  colnames(output)<-c("log2FC","p.value", "Counts/Cell.1","Counts/Cell.2",                               # output dataframe
                      "N.+_cells.1","N.+_cells.2","Counts.1","Counts.2","Sum_w^2.1","Sum_w^2.2")
  ...
  Xi.1 <- t(t(Ci.1)/Ni.1)                                                                                # ident.1 normalized counts
  object.2 <- subset(object, idents = ident.2)
  Ci.2 <- as.matrix(object.2[["RNA"]]$counts)
  Ni.2 <- colSums(Ci.2)
  Nc.2 <- ncol(Ci.2)
  ...
  for (rownum in c(1:length(GeneList))) {                                                                # Main test loop
    ...
    Ncd.1=sum(Xi1!=0)                                                                                    # number of cells expressing the gene
    Ncd.2=sum(Xi2!=0)
    if ((AC.1 >= min.count | AC.2 >= min.count)&                                                         # checking for minimum aggregated counts
        (Ncd.1>=min.cells|Ncd.2>=min.cells)&                                                             # checking for minimum number of cells expressing the gene
        ((Ncd.1/Nc.1>min.pct)|(Ncd.2/Nc.2>min.pct))) {                                                   # checking for minimum fraction of cells expressing the gene
      ...
      wi.2sq<-sum(wi.2^2)
      wi.1<-if(wi.1sq>sum.w2) {replicate(Nc.1,1/Nc.1)} else {wi.1}                                       # switching to equal weights when sum(w^2)>sum.w2 (conservative average and variance calculation when 1 cell appears to dominate)
      wi.2<-if(wi.2sq>sum.w2) {replicate(Nc.2,1/Nc.2)} else {wi.2}
      ...
      if (!is.na(fc)){                                                                                   # removing 0/0 division
        if ((fc >= fc.thr | fc <= 1/fc.thr)) {                                                           # checking FC threshold                                                      
          ...
          if (wTtest <= max.pval) {                                                                      # checking p-value threshold    
            ...
            output[[rowi,5]] <- as.numeric(format(Ncd.1, digits=4, scientific=FALSE))
            output[[rowi,6]] <- as.numeric(format(Ncd.2, digits=4, scientific=FALSE))
            output[[rowi,7]] <- as.numeric(format(AC.1, digits=4, scientific=FALSE))
            output[[rowi,8]] <- as.numeric(format(AC.2, digits=4, scientific=FALSE))
            output[[rowi,9]] <- as.numeric(format(wi.1sq, digits=4, scientific=FALSE))
            output[[rowi,10]] <- as.numeric(format(wi.2sq, digits=4, scientific=FALSE))
...
CntAv<-function(object,features=NULL,icc="i"){
  ...
  for (j in c(1:Ns)){                                                                    # main loop for calculating weighted average and variance sample by sample
    ...
    for (i in c(1:Ng)){                                                                  # gene by gene calculation subloop 
      h<-Ci[GeneList[i],]                                                                # vector of counts for gene i
...
WT.MultiSample <- function(sample.matrix,features=NULL,t.test=FALSE ,min.pct=0.03,fc.thr=1,max.pval=1,df.correction=FALSE,sum.w2=0.4,min.cells=10,min.count=10) {
  ...
  if (is.null(features)){                                                            # check for genes to be analyzed
    ...
    GeneList <- intersect(as.vector(features),rownames(AV1))
  }
  OrphanGenes=setdiff(as.vector(features),rownames(AV1))
  ...
  wi.1eq=replicate(N1,1/N1)                                                          # equal weights for samples in group 1
  wi.2eq=replicate(N2,1/N2)                                                          # equal weights for samples in group 2
  output<-data.frame(p.value=numeric(),log2FC=numeric(),Av1=numeric(),Sd1=numeric(),C1=numeric(),CS1=character(),AGC1=character(),CN1=numeric(),W1=numeric(),
                     Av2=numeric(),Sd2=numeric(),C2=numeric(),CS2=character(),AGC2=character(),CN2=numeric(),W2=numeric())
  colnames(output)<-c("log2FC","p.value","Wtd.%UMI.1","Sd.%UMI.1","Av.cell.fr.1","N.+_cells.1","Counts.1","Counts/cell.1","Sum_w^2.1",
                      "Wtd.%UMI.2","Sd.%UMI.2","Av.cell.fr.2","N.+_cells.2","Counts.2","Counts/cell.2","Sum_w^2.2")
  ...
  for (rownum in c(1:length(GeneList))) {                                            # gene by gene DGE calculation loop
    ...
    CNTi1<-cnt1[GeneList[rownum],]                                                   # count/cell vectors for groups 1 and 2 
    CNTi2<-cnt2[GeneList[rownum],]
    NCDi1<-Ncd1[GeneList[rownum],]                                                   # number of positive cells vectors
    NCDi2<-Ncd2[GeneList[rownum],]
    AGCi1<-Agc1[GeneList[rownum],]                                                   # aggregate count vectors
    AGCi2<-Agc2[GeneList[rownum],]
        ...
        if (fc >= fc.thr | fc <= 1/fc.thr) {                                         # checking FC threshold
          ...
          if(!is.na(wTtest)) {                                                       # output filtering for wTtest=NaN
            if (wTtest <= max.pval) {                                                # output filtering for p-value
              ...
              output[[rowi,6]] <- CellStat1
              output[[rowi,7]] <- CountStat1
              output[[rowi,8]] <- as.numeric(format(mean(CNTi1), digits=4, scientific=FALSE))
              output[[rowi,9]] <- as.numeric(format(mean(wi.1sq), digits=4, scientific=FALSE))
              output[[rowi,10]] <- as.numeric(format(Xi2av, digits=4, scientific=FALSE))
              output[[rowi,11]] <- as.numeric(format(Xi2sd, digits=4, scientific=FALSE))
              output[[rowi,12]] <- as.numeric(format(mean(PCTi2), digits=4, scientific=FALSE))
              output[[rowi,13]] <- CellStat2
              output[[rowi,14]] <- CountStat2
              output[[rowi,15]] <- as.numeric(format(mean(CNTi2), digits=4, scientific=FALSE))
              output[[rowi,16]] <- as.numeric(format(mean(wi.2sq), digits=4, scientific=FALSE))
              row.names(output)[rowi]<-GeneList[rownum]
            } 
          }
          else {
